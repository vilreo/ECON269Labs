...
long.df <- full_join(Rich.df, Middle.df) %>% full_join(Poor.df) %>%
  ...
  group_by(Region) %>%
  mutate(Pop.lag = lag(Population),
         EnInt.lag = lag(EnInt),
         EmInt.lag = lag(EmInt),
         EnInt.g = EnInt/EnInt.lag -1,
         EmInt.g = EmInt/EmInt.lag -1)

PopulationGrowth2020 <- long.df %>% filter(Year == 2020) %>% 
  mutate(Pop.g = Population/Pop.lag - 1) %>% select(Region, Pop.g)

KayaIntensities <- summarise(long.df, 
                           AveEnInt.g = mean(EnInt.g, na.rm = TRUE), 
                           AveEmInt.g = mean(EmInt.g, na.rm = TRUE))

# These values taken from previous calibration exercises
GDPparameters <- data.frame(Region = c("R", "M", "P"),
                            A1960 = c(5.5, 0.65, 0.283), 
                            Agrowth.historic = c(0.0183, 0.0282, 0.0205))
...
```{r}
for (i in 2:length(Pop.df$Year)) {
  Pop.df$Popg.R[i] <- Pop.df$Popg.R[i-1]*.95
  Pop.df$Pop.R[i] <- Pop.df$Pop.R[i-1]*(1 + Pop.df$Popg.R[i])
  
  Pop.df$Popg.M[i] <- Pop.df$Popg.M[i-1]*.95
  Pop.df$Pop.M[i] <- Pop.df$Pop.M[i-1]*(1 + Pop.df$Popg.M[i])
    
  Pop.df$Popg.P[i] <- Pop.df$Popg.P[i-1]*.95
  Pop.df$Pop.P[i] <- Pop.df$Pop.P[i-1]*(1 + Pop.df$Popg.P[i])
}
