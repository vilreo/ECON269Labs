---
title: "Lab 10 Matlab Conversion"
author: "Aaron Swoboda and Oliver Hall"
date: "2025-04-15"
output: pdf_document
---

This file aims to convert a series of Matlab scripts and data files into a R Markdown file that replicates the Tol (2023) Lab 10 for the economics of climate change. https://github.com/rtol/ClimateEconomics 
...
### Step 5 - define fMLIAM10 function
```{r}
fMLIAM10 <- function(ctax, cs) {
  if (any(ctax[1] < 0, ctax[2] < 0, ctax[3] < 0, ctax[4] < 0)) {
    NPV <- 1e+15
  } else {
    NPV <- 0
    ...
    for (s in 1:3) {
      ...
      # optrec <- array(orv, dim = c(NPol, NReg))
      ...
      #   }
      # }
      # for (j in 1:(EndYear-StartPolicy-StepPol*NPol+1)) {
      ...
      for (t in (StartYear+1):EndYear) {
        ...
        if (t > 2010) {
          ...
          popgrowth <- popgrowth * popdecline
          TFPgrowth <- TFPgrowth * TFPdecline
        }
        if (t > 1960) {
      ...
      NPVs <- 0
      for (i in 1:NReg) {
        NPVs <- NPVs + sum(utilDF * welfare[, i])
      }
...
cs <- matrix(c(3.0, 3.0, 3.0, 0.25, 0.50, 0.25), nrow = 2) # exercise 1
# cs <- matrix(c(1.5, 3.0, 4.5, 0.25, 0.50, 0.25), nrow = 2) # exercises 2 and 3

NPV0 <- -fMLIAM10(ctax0, cs)

options <- list(maxfunevals = 1000, maxiter = 1000)
#ctax1 <- optim(ctax0, function(ctax) -fMLIAM10(ctax, cs), method = "Nelder-Mead", control = options)$par
result <- optim(ctax0, function(ctax) -fMLIAM10(ctax, cs), method = "Nelder-Mead", 
