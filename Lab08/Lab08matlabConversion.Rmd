---
title: "Lab 08 Matlab Conversion"
author: "Aaron Swoboda and Oliver Hall"
date: "2025-04-15"
output: pdf_document
---

This file aims to convert a series of Matlab scripts and data files into a R Markdown file that replicates the Tol (2023) Lab 8 for the economics of climate change. https://github.com/rtol/ClimateEconomics 
...
### Step 5 - define fMLIAM08 function
```{r}
fMLIAM08 <- function(ctax) {
  if (ctax < 0) {
    NPV <- 1000000000000000
  } else {
    carbontax <- ctax
   ...
   for (t in (StartYear+1):EndYear) {
      ...
      CO2conc[i] <- sum(MRHbox[i, ])
      RF[i] <- RadForc(CO2conc[i], RF0, CO20)
      temp_and_ocean <- ST(atmtemp[i-1], oceantemp[i-1], RF[i], STpar)
      atmtemp[i] <- temp_and_ocean[1]
      oceantemp[i] <- temp_and_ocean[2]
      impact[i, ] <- impactcc(atmtemp[i], impactpar, 0, 1, 1)
...
options <- list(maxfun = 1000, maxiter = 1000)
ctax1 <- optimize(f = fMLIAM08, interval = c(0, 1000))$minimum
NPV1 <- -fMLIAM08(ctax1)
```
